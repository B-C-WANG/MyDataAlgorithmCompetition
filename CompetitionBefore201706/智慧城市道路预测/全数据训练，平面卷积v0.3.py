...
def build_model2():
    ...
    model.add(Conv2D(
        filters=10,
        kernel_size=(30,132),
        input_shape=(30, 132, 1), padding='same', activation=ACTIVATION
    ))
...
def train2(train=True,load_weights=True,save_weights=True,predict=False,valid=False,epoch=500,per_epoch=100):
    ...
    data_train = data_train.reshape(2206, 30, 132, 1)
    data_question = data_question.reshape(30, 60, 132, 1)[:, 30:, :, :]
    ...
    if train:
        ...
        for i in range(int(epoch/per_epoch)):
            ...
            model.fit(x_train, y_train, batch_size=100, epochs=per_epoch,validation_data=(valid_x,valid_y)
                    ,callbacks=[tb_back])
            ...
            y = scaler.inverse_transform(y)
            y = y.reshape(N,30,132,1)

            abs_loss = np.mean(np.abs(y - valid_y) )
    ...
    if valid:
        ...
        z = np.abs(y-y_train)
        print(z[0,:])
        loss = np.mean((np.abs(y - y_train) / y_train), axis=0)
...
train2(train=True,valid=True,save_weights=True,epoch=20,per_epoch=10,load_weights=True)
