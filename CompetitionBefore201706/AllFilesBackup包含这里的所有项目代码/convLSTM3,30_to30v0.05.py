...
def train2(train=True,load_weights=True,save_weights=True,predict=False,valid=False,epoch=500,batch_size=1000,per_epoch=100):
...
#  只用改变这里，就能够改变输入
    ...
    if train:
        ...
        for i in range(int(epoch/per_epoch)):
            model.fit(x_train_, y_train_, batch_size=batch_size, epochs=per_epoch,validation_data=(x_test_,y_test_)
                    ,callbacks=[tb_back])
...
#train2(train=True,valid=True,save_weights=True,epoch=200,per_epoch=100,load_weights=True)



train2(train=True,valid=True,save_weights=True,epoch=100,
       per_epoch=10,load_weights=True,batch_size=1)
