...
class TensorNode:
    ...
    3. 调用类的方法，建立内部father child结构，相当于第三次初始化
    4. 调用类的方法，建立tf节点，第四次
    5. 进行最后的初始化
    ...
    @classmethod
    def init1(cls,node_shape,node_number):
    ...
    def init2(cls):
        if cls.init1 == 0:
            raise ValueError("Please run cls.init1 to init first")
    ...
    def init3(cls):
        if cls.init2 == 0:
            raise ValueError("Please run cls.init2 to init")
    ...
    def init4(cls):
        if cls.init3 == 0:
            raise ValueError("Please run cls.init3 to init")
    ...
    @classmethod
    def train_and_predict(cls,x_train,save_weights=True,load_weights=True,epoch=200,predict=True,predict_length=20):
        '''
        ...
        '''
        if cls.init4 == 0:
            raise ValueError("Please run cls.init4 to init")
    ...
    def node_update(self,father,child):
        self.father = father
        self.child = child

    def show_info(self):
        print(">>>>>>>>>>>>>>>")
        print("node_name: {}".format(self.name).ljust(15))
        print("father: {}".format(self.father).ljust(15))
        print("child: {}".format(self.child).ljust(15))
    ...
    def get_root(cls):
        ...
        for i in cls.nodes:
            if i.father == None:
                root.append(i)
                print("found root{}".format(i.name).ljust(15))
        return root

    # 根据name或者id找到节点实例
    @classmethod
    def find_node_by_name(cls,name):
        if len(name) == 0:
    ...
    def return_center(self):
        ...
        if self.TI == None:
            ...
            for i in range(len(self.TI)):
                ...
                now = tf.add(tf.multiply(self.TI[i],temp),now)
