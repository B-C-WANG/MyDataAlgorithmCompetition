...
def train2(train=True,load_weights=True,save_weights=True,predict=False,valid=False,epoch=500,per_epoch=100):
    ...
    def myloss2(y_true,y_pred):
        return K.abs((K.mean(K.abs(K.abs(y_pred - y_true) / y_true)))*100)
    ...
    if train:
        ...
        for i in range(int(epoch/per_epoch)):
            ...
            valid_x = x_train[randon_int:randon_int + N, :,:,:,:]
            valid_y = y_train[randon_int:randon_int + N, :, :,:,:]
            valid_y = valid_y.reshape(-1,132)

            valid_y = scaler.inverse_transform(valid_y)
            valid_y = valid_y.reshape(N,30,132,1,1)
            ...
            print((np.abs(y - valid_y) / valid_y)[0,0,:,0,0])
            print((np.abs(y - valid_y) / valid_y)[0, :, 0, 0, 0])
            print((np.abs(y - valid_y) / valid_y)[:, 0, 0, 0, 0])
    ...
    if valid:
        ...
        loss = np.mean(np.abs(np.abs(y - y_train) / y_train))
...
train2(train=True,valid=True,save_weights=True,epoch=50,per_epoch=1,load_weights=True)
