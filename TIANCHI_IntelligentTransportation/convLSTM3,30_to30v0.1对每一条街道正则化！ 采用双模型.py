...
#  只用改变这里，就能够改变输入
    ...
    if train:
        ...
        model2 = build_model2()
        model2.compile(loss=myloss, optimizer="adam")

        x_train_ = model.predict(x_train_)

        if load_weights:
            try:
                model2.load_weights('model_2_weights_30_to_30.h5', by_name=True)
                print("load weights2 successful")
            except:
                pass


        # 二次模型的改变之处，就只有x_train变成了之前predict的结果，其他没有变化

        for j in range(int(epoch/per_epoch)):



            model2.fit(x_train_, y_train_, batch_size=1, epochs=per_epoch,validation_data=(x_test_,y_test_)
                    ,callbacks=[tb_back])

            y = model2.predict(x_test_)
            y = y.reshape((92 - SPLIT_NUM) * 30, 132)
            y = line_normalization_reverse(y)
            ...
            print('\033[1;31;40m \t',"二次模型后x_test的loss", loss,'\033[0m')
            print('\033[1;31;40m \t', "二次模型后x_test的绝对loss", abs_loss, '\033[0m')
            if save_weights:
                model2.save_weights('model_2_weights_30_to_30.h5')
                print("save weights_success!")
...
train2(train=True,valid=True,save_weights=True,epoch=1,per_epoch=1,load_weights=True)
